...
export default function AddBook() {
  ...
  async function selectBook(book) {
    try {
      ...
      setTitle(data.title || "");
      setAuthor(data.authors ? book.authors.map((a) => a.name).join(", ") : "");
    ...
    try {
      ...
      if (
        ...
        const downloadURL = await uploadImage(
          image,
          `books/${savedBookId}.jpg`,
        );
        await updateDoc(doc(database, "books", savedBookId), {
          image: downloadURL,
        });
      }
        ...
        {showMessage && (
          <View style={styles.toast}>
            <Text style={styles.toastText}>
              Book added to 'Wants To Read' under your profile
            </Text>
          </View>
        )}
